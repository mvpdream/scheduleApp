<template>
		<view class="item-pm" :class="[setClass,setBorder]"  @tap="goDetail">{{studentName}}
			<uni-icon v-if="setFinish=='finished'" type="checkmarkempty" size="24" class="item-finish"></uni-icon>
		</view>
</template>
...
<script>
	...
	export default {
				...
				if(this.item_data){
					...
					for (let i = 0; i < curdate.length; i++) {
						if (curdate[i]['times'].indexOf(this.time) > -1) {
							...
							if ((curdate[i]['times'].length==2&&curdate[i]['times'][0].substring(3, 5) == '00')) {
								return 'selectBBorder';
							}else{
								return ''
							}
							break;
		...
		methods: {
			goDetail(){
				this.$emit('wzhclick', {e:this.index,time:this.oldTime,name:this.nameValue});
			}
		}
	}
</script>
